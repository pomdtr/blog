...
I don't really have a usecase for native apps anymore. All of the apps I use are web-based, so it make sense for me to use a web browser as my only opened app. Instead of having a bunch of window with their own set of tabs, I can manage everything from a single app.
...
Ttyd was missing some features that I wanted (ex: automatic light/dark mode, a set of builtin themes), so I created my own project inspired by it, called [tweety](https://github.com/pomdtr/tweety). The rest of this article will focus on this specific project, but you can use the same ideas with ttyd.
